# Room Control Panel API docs

API documentation for Room Control Panel version: `v1.0.x`

- [Introduction](#introduction)
- [Authenticate with Room Control Panel](#authentication)
- [Register a client group - `POST {RCP_URL}/api/v1/clientGroups`](#clientGroups)
- [Working with “users” API - `{RCP_URL}/api/v1/users/integration`](#users)
- [Get rooms data - `{RCP_URL}/api/v1/rooms`](#rooms)
- [Working with “bookings” API - `{RCP_URL}/api/v1/bookings`](#bookings)
- [Get organizer meetings – `GET {RCP_URL}/api/v1/bookings`](#getMeetings)
- [Create a meeting - `POST {RCP_URL}/api/v1/bookings`](#createMeeting)
- [Update a meeting - `PATCH {RCP_URL}/api/v1/bookings/:bookingId`](#updateMeeting)
- [Delete a meeting - `DELETE {RCP_URL}/api/v1/bookings/:bookingId`](#deleteMeeting)
- [Check-in a meeting - `PUT {RCP_URL}/api/v1/bookings/checkedIn?bookingId=:bookingId`](#checkInMeeting)

## <a name="introduction"></a> Introduction

Welcome and thank you for using Room Control Panel (RCP) API!
In this document you can find description of available API enpoints for integrating with Room Control Panel along with parameters specification. You can also find basic examples written in React using Axios library. 
...
Room Control Panel is the server-side software used to manage and monitor your Room managing devices. These devices provide booking functionalities for you. You can display, book, confirm etc. your booking by using them. Room Control Panel also serves as central data repository used to synchronize all your devices. This platform is built on the full stack JavaScript framework [Meteor](https://www.meteor.com/) which uses the DDP protocol to send/receive requests. 

## <a name="authentication"></a> Authenticate with Room Control Panel

Room Control Panel is built on full stack JavaScript framework [Meteor](https://www.meteor.com/) which uses the DDP protocol to communicate. [Restivus](https://github.com/kahmali/meteor-restivus) package is used to expose thiese REST API endpoints.
...
To authenticate, you can only use the username/password of an user of APIIntegration type. You can’t create user of APIIntegration type through Room Control Panel. REST API is designed to be configured by pre-generated API user (defaultDevIntegrationUser) whose password (aka groupToken) can easily be found and copied under "Global Settings" in the application web interface. You can use this credential to create Client Group and other users that will be using this API.
...
## <a name="clientGroups"></a> Register a client group - <sub>`POST {RCP_URL}/api/v1/clientGroups`</sub>

This endpoint is available only for defaultDevIntegrationUser user. You can use it to register a new client group. Endpoint receives only one parameter name -name of client group (e.g. Integration Group). Type of this parameter is string and it is mandatory to provide it. 
...
## <a name="users"></a> Working with “users” API - <sub>`{RCP_URL}/api/v1/users/integration`</sub>
...
### Parameters for `POST {RCP_URL}/api/v1/users/integration` endpoint
...
## <a name="rooms"></a> Get rooms data - <sub>`{RCP_URL}/api/v1/rooms`</sub>
  ...
  "data": [
    {
      ...
      "name": "Room 1",
      "mail": "room1@room.integration.com",
      "address": "room1@room.integration.com",
      "id": "room1",
      "numberOfSeats": 0,
      "alias": "Room 1",
      "isActive": true,
      ...
      "bookings": [
        {
          ...
          "subject": "Open API Booking Example",
    ...
    },
    {
      ...
      "name": "Room 2",
      "mail": "room2@room.integration.com",
      "address": "room2@room.integration.com",
      "id": "room2",
      "numberOfSeats": 0,
      "alias": "room117",
      "isActive": true,
...
## <a name="bookings"></a> Working with “bookings” API - <sub>`{RCP_URL}/api/v1/bookings`</sub>
...
## <a name="getMeetings"></a> Get organizer meetings – <sub>`GET {RCP_URL}/api/v1/bookings`</sub>
  ...
  "data": [
    {
      ...
      "subject": "Room Integration Meeting",
...
## <a name="createMeeting"></a> Create a meeting – <sub>`POST {RCP_URL}/api/v1/bookings`</sub>
...
## <a name="updateMeeting"></a> Update a meeting - <sub>`PATCH {RCP_URL}/api/v1/bookings/:bookingId`</sub>
...
```js
    patchBooking() {
        const bookingData = {
            ...
            subject: "Open API Booking Example Patched",
        };
...
## <a name="deleteMeeting"></a> Delete a meeting - <sub>`DELETE {RCP_URL}/api/v1/bookings/:bookingId`</sub>
...
## <a name="checkInMeeting"></a> Check-in a meeting - `PUT {RCP_URL}/api/v1/bookings/checkedIn?bookingId=:bookingId`
